...
TEST_F(BinaryLoaderTest, MoveAssignmentTransfersOwnership) {
    ...
    // If it doesn't, skip testing a raw shell and just move into another loaded instance.
...
TEST_F(BinaryLoaderTest, GetSectionReturnsNulloptForMissingSection) {
    ...
    // Only test a genuinely fake section name. 
    EXPECT_FALSE(loader.get_section(".this_section_does_not_exist").has_value());
}
