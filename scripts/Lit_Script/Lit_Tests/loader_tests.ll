...
# RUN: not %rosetta %t.fake 2>&1 | FileCheck %s --check-prefix=INVALID

# RUN: not %rosetta %S/this_file_does_not_exist.elf 2>&1 | FileCheck %s --check-prefix=MISSING
...
# RUN: not %rosetta %t.elf 2>&1 | FileCheck %s --check-prefix=WRONGARCH

# RUN: printf '\x7FELF' > %t.truncated
# RUN: not %rosetta %t.truncated 2>&1 | FileCheck %s --check-prefix=TRUNCATED
