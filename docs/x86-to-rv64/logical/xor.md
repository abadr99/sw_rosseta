...
xori    t0, t0, imm8          # t0 = AL XOR imm8
...
xori    t9, t9, 1
```
## XOR RAX, imm32 (opcode REX.W + 35 id)
```asm
# Inputs:   a0 = rax
# Outputs:  a0 = result, {t4-t9} = {CF, OF, AF, SF, ZF, PF}
...
xori    t0, t0, imm8          # t0 = reg XOR imm8
...
xori    t9, t9, 1
```
## XOR r/m16, imm16 (opcode 81 /6 iw)
### Register Destination
```asm
# Inputs:   a0 = x86 register
# Outputs:  a0 = result, {t4-t9} = {CF, OF, AF, SF, ZF, PF}
...
xori    t9, t9, 1
```
## XOR r/m32, imm32 (opcode 81 /6 id)
### Register Destination
```asm
# Inputs:   a0 = x86 register
...
xori    t9, t9, 1
```
## XOR r/m64, imm32 (opcode REX.W + 81 /6 id)
### Register Destination
```asm
...
li      t2, 0xffff
and     t0, a0, t2            # load 16-bit
xori    t0, t0, imm8          # sign-extended imm8
and     t0, t0, t2            # mask
li      t2, -65536
and     a0, a0, t2            # clear low 16
or      a0, a0, t0            # write back
...
xori    t9, t9, 1
```
## XOR r/m64, imm8 (opcode REX.W + 83 /6 ib)
### Register Destination
```asm
# Inputs:   a0 = x86 register (full 64-bit)
# Outputs:  a0 = result, {t4-t9} = {CF, OF, AF, SF, ZF, PF}
...
# Main result
xori    a0, a0, imm8          # sign-extended imm8, 64-bit result
...
xori    t9, t9, 1
```
## XOR r/m64, r64 (opcode REX.W + 31 /r)
### Register Destination
```asm
...
xori    t9, t9, 1
```
## XOR r64, r/m64 (opcode REX.W + 33 /r)
### Register Source
```asm
# Inputs:   a0 = x86 register (destination, full 64-bit), a1 = x86 register (source)
# Outputs:  a0 = result, {t4-t9} = {CF, OF, AF, SF, ZF, PF}
